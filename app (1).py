...
# ── Install dependencies at runtime (guaranteed to work on Streamlit Cloud) ──
def install(pkg):
...
ENCODE_MAP = {
    ...
    'schoolsup':  {'no':0,'yes':1}, 'famsup':     {'no':0,'yes':1},
    'paid':       {'no':0,'yes':1}, 'activities': {'no':0,'yes':1},
    'nursery':    {'no':0,'yes':1}, 'higher':     {'no':0,'yes':1},
    'internet':   {'no':0,'yes':1}, 'romantic':   {'no':0,'yes':1},
}
CLASS_INFO = {
...
# ── Helpers ────────────────────────────────────────────────────────────────────
def predict(model, scaler, raw):
    encoded = [ENCODE_MAP[f][raw[f]] if f in ENCODE_MAP else int(raw[f]) for f in FEATURE_COLS]
    arr = np.array([encoded], dtype=np.float32)
    probs = model.predict_proba(scaler.transform(arr))[0]
    return int(np.argmax(probs)), probs


def risk_factors(raw):
    pos, neg = [], []
...
def radar_fig(raw):
    cats = ['Study Time','Family\nRelation','Health','Free Time','G1 Score','G2 Score']
...
def indicators_fig(raw):
    names = ['G1 Grade','G2 Grade','Study Time ×5','Health ×4','No-Failure Score']
    vals  = [raw['G1'], raw['G2'], raw['studytime']*5, raw['health']*4, (3-raw['failures'])*6]
    fig = go.Figure(go.Bar(
    ...
    with st.sidebar:
        ...
        <div style='text-align:center;padding:1rem 0 0.5rem'>
            ...
            <div style='font-family:Syne,sans-serif;font-weight:800;font-size:1.3rem;
                        color:white;margin-top:0.3rem'>EduPredict AI</div>
        ...
        with st.expander("👤 Personal Information", expanded=True):
            ...
            famsize  = st.selectbox("Family Size",["GT3","LE3"],help="GT3=>3, LE3=≤3 members")
        ...
        st.markdown('<hr><div class="section-header">📝 Mid-Term Grades</div>',
                    unsafe_allow_html=True)
        c1, c2 = st.columns(2)
...
def main():
    ...
    if model is None:
        st.error("**Model files not found.** Ensure `model/student_ann_model.pkl` "
                 "and `model/scaler.pkl` are committed to your repository.")
        st.stop()
    ...
    with tab1:
        if btn or st.session_state.get("predicted"):
            ...
            pred, probs = predict(model, scaler, raw)
            st.session_state.update(p_pred=pred, p_probs=probs, p_raw=raw)
            ...
            with L:
                ...
                for i,(lbl,hx) in enumerate(zip(
                    ...
                    <div class="prob-row">
                        ...
                        <div class="prob-bar-bg">
                            <div class="prob-bar-fill" style="width:{pct:.1f}%;background:{hx}"></div>
                        </div>
                ...
                msg  = "High confidence prediction" if conf>75 else \
                       "Moderate confidence — review borderline factors" if conf>55 else \
                       "Low confidence — borderline case, review carefully"
                st.markdown(f'<div class="tip-box" style="margin-top:1rem">🎯 Confidence: '
                            f'<strong style="color:{cc}">{conf:.1f}%</strong> — {msg}</div>',
                            unsafe_allow_html=True)

            with R:
                st.markdown('<div class="section-header">Student Profile Radar</div>',
                            unsafe_allow_html=True)
                st.plotly_chart(radar_fig(raw), use_container_width=True)
                st.markdown('<div class="section-header">Risk Factor Summary</div>',
                            unsafe_allow_html=True)
                pos, neg = risk_factors(raw)
                for p in pos[:4]:
            ...
            <div style="text-align:center;padding:4rem 2rem;
                ...
                <div style="font-family:Syne,sans-serif;font-size:1.4rem;font-weight:700;
                             color:rgba(255,255,255,0.7);margin-top:1rem">Ready to Predict</div>
    ...
    with tab2:
        if not st.session_state.get("predicted"):
            ...
            r = st.session_state["p_raw"]; pr = st.session_state["p_probs"]
            c1, c2 = st.columns(2, gap="large")
            with c1: st.plotly_chart(prob_fig(pr), use_container_width=True)
            with c2: st.plotly_chart(indicators_fig(r), use_container_width=True)
            st.markdown('<div class="section-header" style="margin-top:1rem">Full Input Summary</div>',
                        unsafe_allow_html=True)
            st.dataframe(
                pd.DataFrame({"Feature":list(r.keys()),"Value":list(r.values())}).set_index("Feature"),
    ...
    with tab3:
        ...
        with c1:
            ...
            <p><strong style="color:#a78bfa">EduPredict AI</strong> classifies students into
            three risk levels based on 32 academic, social and demographic features, enabling
            educators to intervene early before a student falls behind.</p>
            </div>
            ...
            <div style="color:rgba(255,255,255,0.7);font-size:0.88rem;line-height:2.1">
                ...
                → <code style="color:#60a5fa">Dense(256) + Dropout(0.3)</code><br>
                → <code style="color:#60a5fa">Dense(128) + Dropout(0.3)</code><br>
                → <code style="color:#60a5fa">Dense(64)  + Dropout(0.2)</code><br>
        ...
        with c2:
            ...
            for cls, info in CLASS_INFO.items():
                bd = "G3 < 10" if cls==0 else "10 ≤ G3 < 14" if cls==1 else "G3 ≥ 14"
                st.markdown(f"""
                <div class="pred-card {info['color']}" style="padding:1.2rem;margin:0.5rem 0">
                    <div style="display:flex;align-items:center;gap:0.8rem">
                        ...
                        <div style="text-align:left">
                            ...
                            <div class="pred-desc" style="margin:0">Grade: <strong>{bd}</strong><br>{info['desc']}</div>
                        </div>
                    </div>
                </div>""", unsafe_allow_html=True)
            st.markdown("""
