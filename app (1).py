...
.section-header {
    ...
    color: #a78bfa; text-transform: uppercase; letter-spacing: 0.12em;
    margin-bottom: 1rem; padding-bottom: 0.4rem;
    border-bottom: 1px solid rgba(167,139,250,0.25);
}
.pred-card { border-radius: 18px; padding: 2.2rem; text-align: center; margin: 1rem 0; border: 2px solid; }
...
.stTabs [data-baseweb="tab-list"] {
    background: rgba(255,255,255,0.05); border-radius: 12px; padding: 4px; gap: 4px;
}
...
# ── Train model from CSV (runs once, cached forever) ─────────────────────────
@st.cache_resource(show_spinner=False)
def get_model():
    base     = os.path.dirname(os.path.abspath(__file__))
    csv_path = os.path.join(base, "STUDENT_PERFORMANCE_EXPANDED.csv")
    ...
    X = df.drop(columns=["G3", "risk"]).astype(np.float32)
...
# ── Helpers ───────────────────────────────────────────────────────────────────
def run_predict(model, scaler, raw):
    encoded = [
        ENCODE_MAP[f][raw[f]] if f in ENCODE_MAP else int(raw[f])
...
def get_risk_factors(raw):
    ...
    if raw["Medu"] >= 3:         pos.append("✅ Mother has higher education")
...
def make_prob_chart(probs):
    fig = go.Figure(go.Bar(
        x=["At Risk", "Average", "High Performer"],
    ...
    fig.update_layout(
        **PLOTLY_BASE, height=320,
        title=dict(text="Class Probabilities (%)", font=dict(size=13)),
...
def make_indicators(raw):
    names = ["G1 Grade", "G2 Grade", "Study Time x5", "Health x4", "No-Failure Score"]
    vals  = [raw["G1"], raw["G2"], raw["studytime"] * 5,
             raw["health"] * 4, (3 - raw["failures"]) * 6]
    fig = go.Figure(go.Bar(
        x=vals, y=names, orientation="h",
        marker_color=["#60a5fa", "#a78bfa", "#34d399", "#fbbf24", "#f87171"],
    ...
    fig.update_layout(
        **PLOTLY_BASE, height=320,
        title=dict(text="Key Academic Indicators", font=dict(size=13)),
    ...
    with st.sidebar:
        ...
        with st.expander("👤 Personal Information", expanded=True):
            ...
            famsize  = st.selectbox("Family Size", ["GT3", "LE3"],
                                    help="GT3 = more than 3 members, LE3 = 3 or fewer")
        ...
        with st.expander("📚 School & Academic", expanded=True):
            ...
            traveltime = st.slider("Travel Time to School", 1, 4, 1,
                                   help="1=under 15 min  2=15-30 min  3=30-60 min  4=over 60 min")
            studytime  = st.slider("Weekly Study Time", 1, 4, 2,
                                   help="1=under 2h  2=2-5h  3=5-10h  4=over 10h")
        ...
        st.markdown(
            "<hr><div class='section-header'>📝 Mid-Term Grades</div>",
            unsafe_allow_html=True,
        )
        col1, col2 = st.columns(2)
        G1 = col1.number_input("G1 — 1st Period", 0, 20, 10)
        G2 = col2.number_input("G2 — 2nd Period", 0, 20, 10)
...
def main():
    ...
    with st.spinner("Loading model — this takes about 30 seconds on first launch..."):
        model, scaler = get_model()

    raw, btn = render_sidebar()
    ...
    <div class="metric-row">
        ...
        <div class="metric-card">
            ...
            <div class="metric-lbl">Epochs Trained</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    # ── TAB 1 ─────────────────────────────────────────────────────────────────
    with tab1:
        if btn or st.session_state.get("predicted"):
            st.session_state["predicted"] = True
            pred, probs = run_predict(model, scaler, raw)
            st.session_state["p_pred"]  = pred
            st.session_state["p_probs"] = probs
            ...
            with col_l:
                    ...
                    <div class="prob-row">
                        ...
                        <div class="prob-bar-bg">
                            <div class="prob-bar-fill"
                                 style="width:{pct:.1f}%; background:{hx}">
                            </div>
                        </div>
                ...
                conf = max(probs) * 100
                cc   = "#34d399" if conf > 75 else "#fbbf24" if conf > 55 else "#f87171"
                if conf > 75:
                    msg = "High confidence prediction"
                elif conf > 55:
                    msg = "Moderate confidence — review borderline factors"
                else:
                    msg = "Low confidence — borderline case, review carefully"

                st.markdown(
                    f"<div class='tip-box' style='margin-top:1rem'>"
                    f"🎯 Confidence: <strong style='color:{cc}'>{conf:.1f}%</strong>"
                    f" — {msg}</div>",
                    unsafe_allow_html=True,
                )
            ...
            with col_r:
                ...
                st.plotly_chart(make_radar(raw), use_container_width=True)

                st.markdown(
                    "<div class='section-header'>Risk Factor Summary</div>",
                    unsafe_allow_html=True,
                )
                pos, neg = get_risk_factors(raw)
                for p in pos[:4]:
                    st.markdown(
                        f"<div class='tip-box' style='border-color:#34d399'>{p}</div>",
                        unsafe_allow_html=True,
                    )
                for n in neg[:4]:
                    st.markdown(
                        f"<div class='tip-box' style='border-color:#f87171'>{n}</div>",
                        unsafe_allow_html=True,
                    )
        else:
            st.markdown("""
            <div style="text-align:center; padding:4rem 2rem;
                ...
                <div style="color:rgba(255,255,255,0.4); margin-top:0.5rem">
                    ...
                    then click
                    <strong style="color:#a78bfa">PREDICT PERFORMANCE</strong>
                </div>
            </div>
            """, unsafe_allow_html=True)
    ...
    with tab2:
        if not st.session_state.get("predicted"):
            ...
            r  = st.session_state["p_raw"]
            pr = st.session_state["p_probs"]
            c1, c2 = st.columns(2, gap="large")
            with c1:
                st.plotly_chart(make_prob_chart(pr), use_container_width=True)
            with c2:
                st.plotly_chart(make_indicators(r), use_container_width=True)
            st.markdown(
                "<div class='section-header' style='margin-top:1rem'>"
                "Full Input Summary</div>",
                unsafe_allow_html=True,
            )
            st.dataframe(
                pd.DataFrame({
    ...
    with tab3:
        ...
        with c1:
            ...
            <div style="color:rgba(255,255,255,0.7); line-height:1.8; font-size:0.92rem">
                <p>
                    <strong style="color:#a78bfa">EduPredict AI</strong> classifies
                    students into three risk levels based on 32 academic, social and
                    demographic features, enabling educators to intervene early before
                    a student falls behind.
                </p>
                <p>
                    The model trains automatically on first launch using the bundled
                    dataset and is cached for all subsequent predictions.
                </p>
            </div>
            <div class="section-header" style="margin-top:1.5rem">Model Architecture</div>
            <div style="color:rgba(255,255,255,0.7); font-size:0.88rem; line-height:2.1">
                Input (32 features)<br>
                Dense(256) + Dropout(0.3)<br>
                Dense(128) + Dropout(0.3)<br>
                Dense(64)  + Dropout(0.2)<br>
                Dense(32)  + Dropout(0.2)<br>
                Output(3)  Softmax
            </div>
            """, unsafe_allow_html=True)
        ...
        with c2:
            st.markdown(
                "<div class='section-header'>Risk Classes</div>",
                unsafe_allow_html=True,
            )
            for cls, info in CLASS_INFO.items():
                if cls == 0:
                    ...
                    bd = "G3 between 10 and 13"
                else:
                    bd = "G3 14 or above"
                st.markdown(f"""
                <div class="pred-card {info['color']}"
                     style="padding:1.2rem; margin:0.5rem 0">
                    <div style="display:flex; align-items:center; gap:0.8rem">
                        ...
                        <div style="text-align:left">
                            ...
                            <div class="pred-desc" style="margin:0">
                                Grade: <strong>{bd}</strong><br>{info['desc']}
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="metric-row" style="margin-top:1rem">
                ...
                <div class="metric-card">
                    <div class="metric-val">92.0%</div>
                    <div class="metric-lbl">F1-Score</div>
                </div>
                <div class="metric-card">
                    <div class="metric-val">100</div>
                    <div class="metric-lbl">Epochs</div>
                </div>
            </div>
